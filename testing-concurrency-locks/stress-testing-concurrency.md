...
**Scenario:** You wrote the upsert fix from [Race Conditions & Upsert](../database-patterns/race-conditions-upsert.md). Your unit test passes. But does it hold under real concurrency? A single-threaded PHPUnit test won't reproduce the race condition - you need multiple processes hitting the same row simultaneously.
